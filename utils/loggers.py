...
class CsvLogger:
    def __init__(self, setting_str: str, dataset_str: str,
        ...
        self.canonical_metrics = {
            ...
            "class_il_macro_f1": [],
            "task_il_macro_f1": [],
            "class_il_weighted_f1": [],
            "task_il_weighted_f1": [],
            "class_il_auroc": [],
            "class_il_kappa": [],
            "task_il_kappa": [],
            "global_seen_bacc": [],
            "global_seen_macro_f1": [],
            "global_seen_weighted_f1": [],
            "global_seen_kappa": [],
        }
        ...
        self.acc_fwt = np.nan
        self.micro_acc_fwt = np.nan
        self.auc_fwt = np.nan
        self.acc_fwt_mask_classes = np.nan
        self.micro_acc_fwt_mask_classes = np.nan
    ...
    def log_result(self, result) -> None:
        """Log a named EvaluationResult without relying on tuple positions."""
        self.log(
            result.class_il_macro_accuracy,
            result.task_il_macro_accuracy,
            result.class_il_micro_accuracy,
            result.task_il_micro_accuracy,
            result.task_macro_auc,
            result.global_seen_auc,
        )
        values = {
            "class_il_bacc": np.nanmean(result.class_il_balanced_accuracy),
            "task_il_bacc": np.nanmean(result.task_il_balanced_accuracy),
            "class_il_macro_f1": np.nanmean(result.class_il_macro_f1),
            "task_il_macro_f1": np.nanmean(result.task_il_macro_f1),
            "class_il_weighted_f1": np.nanmean(result.class_il_weighted_f1),
            "task_il_weighted_f1": np.nanmean(result.task_il_weighted_f1),
            "class_il_auroc": np.nanmean(result.class_il_auc),
            "class_il_kappa": np.nanmean(result.class_il_kappa),
            "task_il_kappa": np.nanmean(result.task_il_kappa),
            "global_seen_bacc": result.global_seen_metrics["bacc"],
            "global_seen_macro_f1": result.global_seen_metrics["macro_f1"],
            "global_seen_weighted_f1": result.global_seen_metrics["weighted_f1"],
            "global_seen_kappa": result.global_seen_metrics["kappa"],
        }
        for key, value in values.items():
            self.canonical_metrics[key].append(float(value))
    ...
    def write(self, args: Dict[str, Any]) -> None:
        ...
        for metric_name, values in self.canonical_metrics.items():
            args[f"{metric_name}_last"] = format(values[-1], ".4f")
            args[f"{metric_name}_mean"] = format(np.nanmean(values), ".4f")
            new_cols.extend([f"{metric_name}_last", f"{metric_name}_mean"])
        ...
        for metric_name, values in self.canonical_metrics.items():
            print(f'{metric_name + "_last":28s}{values[-1]:.4f}')
            print(f'{metric_name + "_mean":28s}{np.nanmean(values):.4f}')
