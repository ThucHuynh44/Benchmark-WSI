...
class Instance_Buffer:
    """Reservoir of individual patches, indexed by global class label."""

    attributes = ("examples0", "examples1", "patch_sizes", "labels")

    def __init__(self, buffer_size, device, n_tasks=None, mode="reservoir"):
        if mode != "reservoir":
            raise ValueError("Instance_Buffer supports reservoir mode only")
    ...
    def _initialize(self, features, coords):
        self.examples0 = torch.zeros(
            (self.buffer_size, *features.shape[1:]), dtype=features.dtype, device=self.device
        )
        self.examples1 = torch.zeros(
            (self.buffer_size, *coords.shape[1:]), dtype=coords.dtype, device=self.device
        )
        self.patch_sizes = torch.zeros(
            (self.buffer_size,), dtype=torch.long, device=self.device
        )
        self.labels = torch.full((self.buffer_size,), -1, dtype=torch.long, device=self.device)

    def add_data(self, examples, labels=None, logits=None, task_labels=None):
        if len(examples) != 3:
            raise ValueError("Instance_Buffer examples require features, coords, and patch size")
        features, coords, patch_size = [item.to(self.device) for item in examples]
        if labels is None:
            raise ValueError("Instance_Buffer requires global labels")
        if not hasattr(self, "examples0"):
            self._initialize(features, coords)
        global_label = int(torch.as_tensor(labels).reshape(-1)[0].item())
        for patch_index in range(features.shape[0]):
            index = reservoir(self.num_seen_examples, self.buffer_size)
            if index >= 0:
                self.examples0[index] = features[patch_index]
                self.examples1[index] = coords[patch_index]
                self.patch_sizes[index] = torch.as_tensor(patch_size).reshape(-1)[0].long()
                self.labels[index] = global_label
            self.num_seen_examples += 1

    def available_labels(self, labels: Iterable[int]):
        if self.is_empty():
            return []
        valid = self.labels[:len(self)]
        return [int(label) for label in labels if torch.any(valid == int(label))]

    def sample_class(self, label: int, size: int):
        if self.is_empty():
            raise ValueError("Cannot sample an empty Instance_Buffer")
        valid_labels = self.labels[:len(self)]
        indexes = torch.nonzero(valid_labels == int(label), as_tuple=False).flatten()
        if indexes.numel() == 0:
            raise ValueError(f"No buffered patches for global class {label}")
        sample_size = min(int(size), int(indexes.numel()))
        choice = indexes[torch.randperm(indexes.numel(), device=indexes.device)[:sample_size]]
        patch_size = self.patch_sizes[choice[0]]
        return self.examples0[choice], self.examples1[choice], patch_size

    def get_task_bags(self, class_labels: Iterable[int], size: int):
        bags = []
        for label in self.available_labels(class_labels):
            features, coords, patch_size = self.sample_class(label, size)
            bags.append((features, coords, patch_size, label))
        return bags
