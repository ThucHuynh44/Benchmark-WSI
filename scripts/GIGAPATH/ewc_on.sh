#!/bin/bash
#SBATCH --job-name=ewc_on_gigapath
#SBATCH --output=logs/GIGAPATH/ewc_on_%j.out
#SBATCH --error=logs/GIGAPATH/ewc_on_%j.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
MODEL="${MODEL:-ewc_on}"
FOLDS="${FOLDS:-all}"
REPO_ROOT=/datastore/uittogether/LuuTru/Thuchd/benchmarkWSI/Benchmark-WSI

mkdir -p "$REPO_ROOT/logs/GIGAPATH"
...
conda activate benchmark_wsi310

cd "$REPO_ROOT"

export PYTHONUNBUFFERED=1
# Prov-GigaPath is pinned to PyTorch 2.0.0, whose CUDA allocator does not
# support the newer expandable_segments option. Clear any inherited value so
# CUDA can initialize normally on the Slurm node.
...
python -u utils/main.py \
    --config configs/methods.yaml \
    --model "$MODEL" \
    --backbone gigapath \
    --dataset_config configs/datasets_gigapath.yaml \
    --folds "$FOLDS"

echo "Hoan thanh Prov-GigaPath."
