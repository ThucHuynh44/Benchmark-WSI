#!/bin/bash
#SBATCH --job-name=atlasV3MeanOnly
#SBATCH --output=logs/FEATHER/atlasV3MeanOnly_%j.out
#SBATCH --error=logs/FEATHER/atlasV3MeanOnly_%j.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
VARIANT=atlasv3_acl_gated_transport_normalized_oas_no_histneg_coverage_only_mean_only
...
python -u scripts/summarize_atlas_v3_acl_ablations.py \
    ...
    "$VARIANT" \
    --output results/ablations/atlas_v3_acl/summary_covariance_propagation \
    --percent --strict

echo "Hoan thanh ASTRA mean-only covariance-propagation ablation."
