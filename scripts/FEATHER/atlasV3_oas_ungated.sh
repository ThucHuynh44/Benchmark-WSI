#!/bin/bash
#SBATCH --job-name=atlasV3OASUngated
#SBATCH --output=logs/FEATHER/atlasV3OASUngated_%j.out
#SBATCH --error=logs/FEATHER/atlasV3OASUngated_%j.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
# Pilot ba folds theo mac dinh. Dat FOLDS=all khi muon chay du 10 folds.
FOLDS="${FOLDS:-0-2}"
REPO_ROOT=/datastore/uittogether/LuuTru/Thuchd/benchmarkWSI/version_moi/Benchmark-WSI/

# Hai variant nay khong trung voi oas_static/oas_oracle cua job dang chay.
VARIANTS=(
...
echo "Hoan thanh Frozen raw-OAS control va ACL ungated transport OAS."
