#!/bin/bash
#SBATCH --job-name=atlasV3GateSweep
#SBATCH --output=logs/FEATHER/atlasV3GateSweep_%j.out
#SBATCH --error=logs/FEATHER/atlasV3GateSweep_%j.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
FOLD="${FOLD:-0}"
SWEEP_GROUP="${SWEEP_GROUP:-strength}"
...
if ! [[ "$FOLD" =~ ^[0-9]$ ]]; then
    echo "ERROR: FOLD must be one integer in 0..9; got '$FOLD'." >&2
    exit 2
fi
...
echo "SWEEP_SETTINGS=${SWEEP_SETTINGS:-<group-default>}"
echo "FOLD=$FOLD"
nvidia-smi -i "$BEST_GPU"

RUN_COMMAND=(
    python -u scripts/run_atlas_v3_gated_transport_sweep.py
    "$ACTION"
    --fold "$FOLD"
)
if [[ -n "$SWEEP_SETTINGS" ]]; then
    RUN_COMMAND+=(--settings "$SWEEP_SETTINGS")
else
    RUN_COMMAND+=(--groups "$SWEEP_GROUP")
fi
...
if [[ -n "$SWEEP_SETTINGS" ]]; then
    echo "Targeted settings completed; summarize all pilot folds after the jobs finish."
else
    python -u scripts/summarize_atlas_v3_gated_transport_sweep.py \
        --groups "$SWEEP_GROUP" \
        --folds "$FOLD"
fi

echo "Hoan thanh ATLAS-v3 gated normalized-OAS sweep group=$SWEEP_GROUP fold=$FOLD."
