#!/bin/bash
#SBATCH --job-name=atlasV3GateParts
#SBATCH --output=logs/FEATHER/atlasV3GateParts_%j.out
#SBATCH --error=logs/FEATHER/atlasV3GateParts_%j.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
python -u scripts/summarize_atlas_v3_acl_ablations.py \
    --variants "${VARIANTS[@]}" \
    --output results/ablations/atlas_v3_acl/summary_reliability_gate_components \
    --percent

echo "Hoan thanh ATLAS-v3 reliability-gate component ablation."
