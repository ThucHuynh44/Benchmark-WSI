"""Generate, audit, and optionally run fold-isolated ATLAS-v2 settings."""
...
PILOT_VARIANTS = (
    "atlasv2_base_frozen", "atlasv2_base_lora", "atlasv2_lora_replay",
...
def experiment_desc(registry: Dict[str, Any], variant_id: str, fold: int) -> str:
    root = str(registry["defaults"].get("result_root", "ablations/atlas_v2")).strip("/")
...
def build_command(registry: Dict[str, Any], variant: Dict[str, Any], fold: int) -> list[str]:
    ...
    command = [
        ...
        "--model", "atlas_v2", "--backbone", "feather", "--folds", str(int(fold)),
        "--exp_desc", experiment_desc(registry, variant["id"], fold),
        "--ablation_id", variant["id"], "--ablation_group", variant["group"],
...
def resolved_audit(variant: Dict[str, Any], fold: int) -> str:
    ...
    replay = bool(values["atlasv2_replay"])
    return (
        f"variant={variant['id']} fold={int(fold)} "
        f"LoRA={'ON' if values['atlasv2_lora'] else 'OFF'} "
        f"Replay={'ON' if replay else 'OFF'} buffer={30 if replay else 0} "
        f"full_bag={'YES' if replay else 'N/A'} "
...
def inspect_run(registry: Dict[str, Any], variant: Dict[str, Any], fold: int) -> str:
    ...
    counts = [
        _read_keys(run_dir / f"evaluation/{mode}/eval_matrix.csv")
        for mode in ("class_il", "task_il")
    ]
    complete = all(
        set(values) == expected and all(count == 1 for count in values.values())
...
def _run_job(job, gpu: str | None):
    ...
    print(f"[atlas-v2] start variant={variant['id']} fold={fold} gpu={gpu or 'inherited'}", flush=True)
    result = subprocess.run(command, cwd=REPO_ROOT, env=environment, check=False)
    print(f"[atlas-v2] end variant={variant['id']} fold={fold} exit={result.returncode}", flush=True)
    return variant["id"], int(fold), int(result.returncode)


def _execute(jobs, gpus: Sequence[str]) -> int:
    if not jobs:
        print("No ATLAS-v2 fold-runs need execution.")
        return 0
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--registry", default=str(REPO_ROOT / "configs/atlas_v2_ablations.yaml")
    )
