"""Generate, audit, and run fold-isolated ATLAS-v3 settings."""
...
from scripts.atlas_v3_registry import SETTING_IDS, load_registry, select_variants
...
def experiment_desc(registry: Dict[str, Any], variant_id: str, fold: int) -> str:
    root = str(registry["defaults"].get("result_root", "ablations/atlas_v3"))
    return f"{root.strip('/')}/{variant_id}/fold_{int(fold)}"


def build_command(
    registry: Dict[str, Any], variant: Dict[str, Any], fold: int
) -> list[str]:
    ...
    command = [
        ...
        "utils/main.py",
        "--config",
        str(defaults.get("config", "configs/methods.yaml")),
        "--model",
        "atlas_v3",
        "--backbone",
        "feather",
        ...
        str(int(fold)),
        "--exp_desc",
        experiment_desc(registry, variant["id"], fold),
        "--ablation_id",
        variant["id"],
        "--ablation_group",
        variant["group"],
...
def resolved_audit(variant: Dict[str, Any], fold: int) -> str:
    mode = variant["overrides"]["atlasv3_distribution_mode"]
    return (
        f"variant={variant['id']} fold={int(fold)} "
        f"FEATHER=FROZEN Training=STATISTICS_ONLY Classifier={mode} Memory=NONE"
    )
...
def inspect_run(
    ...
    complete = all(
        set(values) == expected
        and all(count == 1 for count in values.values())
...
def _run_job(job, gpu: str | None):
    ...
    print(
        f"[atlas-v3] start variant={variant['id']} fold={fold} "
        f"gpu={gpu or 'inherited'}",
        flush=True,
    )
    result = subprocess.run(command, cwd=REPO_ROOT, env=environment, check=False)
    print(
        f"[atlas-v3] end variant={variant['id']} fold={fold} "
...
def _execute(jobs, gpus: Sequence[str]) -> int:
    if not jobs:
        print("No ATLAS-v3 fold-runs need execution.")
        return 0
    ...
    for variant_id, fold, code in failures:
        print(
            f"FAILED variant={variant_id} fold={fold} exit={code}",
            file=sys.stderr,
        )
    ...
    for variant in variants:
        for fold in folds:
            ...
            if args.action == "resume":
                ...
                if status == "mismatch":
                    raise RuntimeError(
                        f"Identity mismatch for {variant['id']} fold {fold}"
                    )
                if status == "incomplete" and not args.rerun_incomplete:
                    raise RuntimeError(
                        "Pass --rerun-incomplete to replace an incomplete run"
                    )
